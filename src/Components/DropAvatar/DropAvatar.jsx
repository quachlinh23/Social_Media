...
export default function DropAvatar({ User }) {
  const nagative = useNavigate(); // khai báo hook chuyển trang
  const { logout } = useAuth(); // lấy hàm logout từ context

  //Hàm xử lý đăng xuất
  function HandleLogout() {
    logout();
    nagative("/login"); //Chuyển ra trang đăng nhập
  }
    ...
    <div className="dropDownAvatar">
      <ul className="OptionLists">
        ...
        <Link to={`/profile/${User.id}`} className="noLinkStyle">
          <li className="OtpionProfile" title="Truy cập trang cá nhân">
            <img src={User.profilePicture} alt="" className="OptionImg" />
            <span className="optionName">{User.fullname}</span>
          </li>
        </Link>
